parameter_tabs <- tabsetPanel(
  ...
  tabPanel(
    "Gamma",
    numericInput("shape", "shape", value = 5),
    numericInput("scale", "scale", value = 1)
  ),
  tabPanel(
    "Log-normal",
    numericInput("meanlog", "meanlog", value = 1, min = 0),
...
cfr_options_tabs <- tabsetPanel(
  ...
  tabPanel(
    "rolling",
    numericInput("poisson_threshold", "poisson_threshold", value = 100, min = 1)
  ),
  tabPanel(
    "time_varying",
    numericInput("burn_in", "burn_in", value = 7, min = 0),
    numericInput("smoothing_window", "smoothing_window", value = 1, min = 1)
  )
)
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      cfr_options_tabs,
      checkboxInput(
        "correct_delays", "Correct for delays?"
      ),
      conditionalPanel(
    ...
    mainPanel(
      tableOutput("cfr_overall"),
      plotlyOutput("cfr_plot"),
      conditionalPanel(
        "input.correct_delays",
        div(
          style = "display:flex;",
          plotlyOutput("plot_pmf", width = "25vw", height = "25vw"),
          plotlyOutput("plot_cdf", width = "25vw", height = "25vw")
        )
      )
    )
  )
