...
Then we define a global function `estimate_func()` in `global.R` that estimates R using `{EpiEstim}`. As a placeholder, we assume mean serial interval = 5 days with s.d. = 1. We then edit `transmission_server()` in `01b_transmission.R` to output deaths from earlier CFR calculation, then use provide these numbers required for R estimation:
```
...
```
**Question: is this the optimal place to introduce this functionality (i.e. using ratio as placeholder for timeseries)**

**Issue suggestion: seems to be some duplication of plotting within the `(isolate(isTruthy(input$show_ratio_line)))` if statement, which makes customisation more challenging. Also unclear how to display R by default, rather than requiring user to select Options > Show R? in plot**
...
- **Issue suggestion (for EpiEstim): option to complete the output vector of moving average with NA values, to save additional coding to match vector lengths (see `estimate_func()` above)**
