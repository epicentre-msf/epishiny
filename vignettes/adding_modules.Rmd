...
Then we define a global function `estimate_func()` in `global.R` that estimates R using `{EpiEstim}`. We then edit `transmission_server()` in `01b_transmission.R` to output deaths from earlier CFR calculation, then use provide these numbers required for R estimation:
```
...
**Issue suggestion: seems to be some duplication of plotting within the `(isolate(isTruthy(input$show_ratio_line)))` if statement, which makes customisation more challenging.**
